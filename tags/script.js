...
riot.tag2('main', '<chats></chats> <form class="message-form" id="chat-form"> <input class="message-input" type="text" name="text" placeholder="Type a message.." required> <button class="btn btn-send" type="submit">Send</button> </form>', '', '', function(opts) {
});
riot.tag2('navibar', '<nav class="navbar"> <h2 class="title">ChatsApp</h2> <input class="navbar-input" id="searchChat" type="text" placeholder="Search in chats" onchange="filterChat()"> <div class="dropdown"> <button class="btn btn-dropdown" id="usernameBtn" onclick="account()"></button> <button class="btn btn-dropdown" onclick="settings()">Settings</button> <button class="btn btn-dropdown btn-danger" onclick="logOut()">Logout</button> </div> </nav>', '', '', function(opts) {
});

riot.tag2('sidenav', '<input class="sidenav-input" id="searchUser" type="text" placeholder="Search" onchange="filterUser()"> <h3 style="text-align: center;margin: 5px;">Registered Users</h3> <div class="userWrap"> <ul class="sidenav-list" id="user_list"> </ul> </div> <div class="sidenav-footer"> <h6>Invite Report About<h6> <h6>&copy; 2018 ChatsApp</h6> </div>', '', '', function(opts) {
});
